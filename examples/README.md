...
3. **On the `star_rsem` branch, `RSEM_CALCULATEEXPRESSION` is over-requested and too slow for its time limit.**
   - **Since STAR moved out of the task** (3.21.0, nf-core/rnaseq #1604), RSEM tasks average about one core (median 103% over 39 completed tasks) and peak at or below 9.45 GiB while requesting 12 CPUs / 72 GiB / 16 h. In 3.22.0 that one process is 75% of a $294 run at 9% CPU efficiency.
   - **The full-size test failed in 3.23.0, 3.24.0 and 3.25.0** because an RSEM task hit the 16 h limit (`Job attempt duration exceeded timeout`, per each run's report). In 3.23.0 and 3.24.0 the task's last output was still `rsem-parse-alignments` progress (`Parsed 255000000` / `73000000 entries`), RSEM's single-threaded first step.
   - **In 3.27.0** it passed; one task needed three attempts (exit 175, Fusion Snapshots errors), and two tasks ran past the 16 h limit without being stopped (22.6 h, 19.4 h). RSEM is 89% of that $464 run.
   - **Locally the same command does use its threads** (4.36× faster at 12 than at 1), and the single-threaded parse step takes 98 s for 18 M entries (see `rsem-threads/`). Why parsing is so much slower on the test infrastructure is not established.
