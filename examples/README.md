...
3. **The `star_rsem` branch regressed.** `RSEM_CALCULATEEXPRESSION` per sample went from 2–2.7 cores, 30 GB, 3.7–6.7 h in 3.17.0 to about 1 core, 7–9 GB, 7.7–11 h in 3.22.0 while still requesting 12 CPUs and 72 GB for 16 h. That one process is 75% of a $294 run at 9% CPU efficiency; the branch costs 3.8× the `star_salmon` branch on the same samples, and 81–84% of what it is billed for goes unused. Right-sizing it to 2 CPUs / 12 GB is the single largest saving in this corpus. Why RSEM lost its threads between 3.17 and 3.18 is not established here; the numbers are.
